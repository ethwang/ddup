...
- 堆内存管理
  ...
  - 收集器(垃圾回收) 
    ...
    - 三色标记法：(白色对象：被回收对象；灰色对象：中间状态，会扫描灰色对象引用的子对象；黑色对象：可达对象，不被回收；)
      1. GC从根对象集合（包括全局变量、堆栈上的局部变量等）开始，把根对象标记为灰色，然后把这些根对象的引用（子对象）扫描并标记。
      2. 从灰色对象集合中选择一个灰色对象并标记成黑色；
  ...
  https://onlinetool-mauve.vercel.app/develop/memory/gc_barrier.md
  https://www.cnblogs.com/flippedxyy/p/15558742.html
- 栈内存管理
