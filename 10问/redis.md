...
8. redis集群模式(数据分片存储+多副本模式)
    ...
    - 在集群中执行命令
        ...
        - 保存槽和键的关系(clusterState中的一个跳表结构保存槽和键的关系)
            - 跳表每个节点的分值是槽号,每个节点成员是数据键(一个槽对应多个键),方便的对某个槽的所有数据库键进行批量操作
...
11. redis集群扩缩容为啥不用一致性哈希算法? 
  - 哈希槽vs一致性哈希
    - 一致性哈希更容易引起缓存雪崩,当其中一个节点挂掉时，一致性哈希会顺时针将请求打到下一个节点。下一个节点就会承载更多的流量而导致被打挂，接着后面的节点承载更多流量被打挂，引起雪崩。
    - 如果热key所在哈希槽的节点被打挂，会在从节点中进行重新的选主操作，而不会引发雪崩连锁反应。