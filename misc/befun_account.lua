...
function _M.rewrite(conf, ctx)
    ...
    if ((not uid) and (not token)) then
        ...
        elseif (uid) then
            if vc == "1000000" then
                core.log.info("vc: ", vc)
                local key1 = "accountapi_uid_2_token_web:{" .. uid .. "}"
                local key2 = "accountapi_token_2_uid_web:{" .. uid .. "}:" .. did ..
                                 ":" .. token
                local res, err = red_c:eval(evalstr, 2, key1, key2,
                                            did .. ":" .. token,
                                            tostring(ngx.time() * 1000))
                if err then
                    ngx.log(ngx.ERR, "red_c failed:", err, "key1=", key1,
                            "key2=", key2)
                    core.response.add_header("Content-Type", "application/grpc")
                ...
                local key2 = "accountapi_token_2_uid:{" .. uid .. "}:" .. did ..
                                 ":" .. token
                local res, err = red_c:eval(evalstr, 2, key1, key2,
                                            did .. ":" .. token,
                                            tostring(ngx.time() * 1000))
